...
import * as listingsStorage from '../../services/storage/listingsStorage.js';
...
const NOT_ASSOCIATED_MESSAGE = 'You are trying to remove a job that is not associated to your user';
...
jobRouter.delete('', async (req, res) => {
  ...
  try {
    ...
    if (!doesJobBelongsToUser(job, req)) {
      res.send(new Error(NOT_ASSOCIATED_MESSAGE));
    } else {
...
jobRouter.get('/:jobId/clearhistory', async (req, res) => {
  const { jobId } = req.params;
  try {
    const job = jobStorage.getJob(jobId);
    if (!doesJobBelongsToUser(job, req)) {
      ...
      listingsStorage.removeListings(jobId);
    }
  } catch (error) {
    res.send(new Error(error));
    console.error(error);
...
jobRouter.get('/:jobId/run', async (req, res) => {
  const { jobId } = req.params;
  try {
    const job = jobStorage.getJob(jobId);
    if (!doesJobBelongsToUser(job, req)) {
